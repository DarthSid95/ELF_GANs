...
********** Baseline WGANs ************************************************************
***********************************************************************************'''
class WGAN_ImNoise2Im(GAN_ImNoise2Im):

	def __init__(self,FLAGS_dict):

		# self.KLD_flag = KLD_flag
		# self.KLD = []
		GAN_ImNoise2Im.__init__(self,FLAGS_dict)
	...
	def train(self):    
		...
		for epoch in range(start,self.num_epochs):
			...
			for image_batch, noise_batch in zip(self.train_dataset, self.noise_dataset):
				self.total_count.assign_add(1)
				# batch_count.assign_add(self.Dloop)
				batch_count.assign_add(1)
				start_1 = time.time()
				...
				with tf.device(self.device):
					self.train_step(image_batch, noise_batch)
					self.eval_metrics()
					# if self.KLD_flag and self.total_count.numpy()%10 == 0 :
					# 	self.updateKLD()
					# 	self.res_file.write("Gaussian KLD: {:>2.6f} \n".format(self.KLD[-1]))
	...
	def train_step(self,reals_all, noiseIms_all):
		for i in tf.range(self.Dloop):
			self.noise = noiseIms_all
			self.reals = reals_all
